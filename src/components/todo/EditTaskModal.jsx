...
export function EditTaskModal({ todo, onUpdateTask, trigger }) {
  ...
  // Initialize form data when todo changes or modal opens
  useEffect(() => {
    if (todo && open) {
      setFormData({
        title: todo.title || "",
        ...
        dueDate: parseDateFromAPI(todo.due_date),
        tagNames: todo.tags
          ? todo.tags.map((tag) => (typeof tag === "string" ? tag : tag.name))
          : [],
      });
      setErrors({});
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const taskData = {
        ...
        due_date: formatDateForAPI(formData.dueDate),
        tag_names: formData.tagNames,
      };

      // Call the parent component's update task function
      await onUpdateTask(todo.id, taskData);
  ...
  // Reset form to original todo data
  const resetForm = () => {
    if (todo) {
      setFormData({
        title: todo.title || "",
